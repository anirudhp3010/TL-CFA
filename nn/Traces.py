import sys
import pdb
from SimpleTree import SimpleTree, Formula, DecisionTreeFormula
import io
import re
import itertools
import contextlib
...
def lineToTrace(line):
    ...
    traceVector = [[varValue for varValue in varsInTimestep.split(',')] for varsInTimestep in
                   traceData.split(';')]
    trace = Trace(traceVector, **kwargs)
    numVariables = len(traceVector[0])
    return trace, traceVector, numVariables
...
class Trace:
    ...
    def evaluateFormulaOnTraceSTL(self, formula,timestep):

            if isinstance(formula, Formula):

                    
                truthValue = self.STLtruthValue(formula, timestep)
                    
            return truthValue
            
    
    def STLtruthValue(self, formula, timestep):
        if timestep is None:
            return False
        ...
        label = formula.label
        if label == '&':
                return int(bool(self.STLtruthValue(formula.left, timestep) and self.STLtruthValue(formula.right, timestep)))
        elif label == '|':
                return int(bool(self.STLtruthValue(formula.left, timestep) or self.STLtruthValue(formula.right, timestep)))
        elif label == '!':
                return int(bool(not self.STLtruthValue(formula.left, timestep)))
        elif label == '->':
                return int(bool(not self.STLtruthValue(formula.left, timestep) or self.STLtruthValue(formula.right, timestep)))
        elif re.search(r'F\[\d+,\d+\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                 if timestep+result[1] <  self.lengthOfTrace:
                   return max([self.STLtruthValue(formula.left, futureTimestep) for futureTimestep in range(timestep+result[0],timestep+result[1]+1)])
                 else:
                   return 0
        elif re.search(r'G\[\d+,\d+\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                 if timestep+result[1] <  self.lengthOfTrace:
                   return min([self.STLtruthValue(formula.left, futureTimestep) for futureTimestep in range(timestep+result[0],timestep+result[1]+1)])
                 else:
                   return 0
        elif re.search(r'U\[\d+,\d+\]',label):
                pattern = r'\d+'
                result = [int(s) for s in re.findall(pattern, label)]
                if timestep+result[1] <  self.lengthOfTrace:
                 return max(
                    [self.STLtruthValue(formula.right, futureTimestep) for futureTimestep in range(timestep+result[0],timestep+result[1]+1)]) == True \
                       and ( \
                                   self.STLtruthValue(formula.right, timestep+timestep+result[0]) \
                                   or \
                                   (self.STLtruthValue(formula.left, timestep+timestep+result[0]) and self.STLtruthValue(formula,
                                                                                                self.nextPos(timestep)+timestep+result[0])) \
                           )
                else:
                    return 0
        elif  re.search(r'x[0-9]*>\d',label):
                  label = label.split(">")
                  dim = (re.findall('[0-9]+', label[0]))
                  dim = int(dim[0])
                  num = float(label[1])
                  if timestep<self.lengthOfTrace:
    
                   return int(bool((float(self.traceVector[timestep][dim]))>num))
                  else: 
                   return 0
                  ...
                  dim = int(dim[0])
                  num = float(label[1])
                  #print('tr',float(self.traceVector[timestep][dim]))
                  #print('num',num)
...
class Time:
    ...
    def Sat_Best_Time(self, formula):
        
            label = formula.label
            if label == '&':
                return max(self.Sat_Best_Time(formula.left), self.Sat_Best_Time(formula.right))
            elif label == '|':
                return min(self.Sat_Best_Time(formula.left), self.Sat_Best_Time(formula.right))
            elif label == '!':
                return self.Vio_Best_Time(formula.left)

            elif re.search(r'F\[\d,\d\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                  
                 return self.Sat_Best_Time(formula.left)+result[0]
            elif re.search(r'G\[\d,\d\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                  
                 return self.Sat_Best_Time(formula.left)+result[1]
          
            elif label == 'true':
                return 0
            elif label == 'false':
                return 0
            elif re.search(r'x\d',label):
                return 0
            else:
                raise NotImplementedError(f"evaluation of operator {label!r}")  
    def Sat_Worst_Time(self, formula):
        
            label = formula.label
            if label == '&':
                return max(self.Sat_Worst_Time(formula.left), self.Sat_Worst_Time(formula.right))
            elif label == '|':
                return max(self.Sat_Worst_Time(formula.left), self.Sat_Worst_Time(formula.right))
            elif label == '!':
                return self.Vio_Worst_Time(formula.left)

            elif re.search(r'F\[\d,\d\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                  
                 return self.Sat_Worst_Time(formula.left)+result[1]
                ...
                raise NotImplementedError(f"evaluation of operator {label!r}")    
    def Vio_Best_Time(self, formula):
        
            label = formula.label
            if label == '&':
                return min(self.Vio_Best_Time(formula.left), self.Vio_Best_Time(formula.right))

            elif label == '|':
                return max(self.Vio_Best_Time(formula.left), self.Vio_Best_Time(formula.right))

            elif label == '!':
                 
                return self.Sat_Best_Time(formula.left)

            elif re.search(r'F\[\d,\d\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                  
                 return self.Vio_Best_Time(formula.left)+result[1]

            elif re.search(r'G\[\d,\d\]',label):
                 pattern = r'\d+'
                 result = [int(s) for s in re.findall(pattern, label)]
                  
                 return self.Vio_Best_Time(formula.left)+result[0]
                ...
                raise NotImplementedError(f"evaluation of operator {label!r}")   
    def Vio_Worst_Time(self, formula):
        
            label = formula.label
            if label == '&':
                return max(self.Vio_Worst_Time(formula.left), self.Vio_Worst_Time(formula.right))
            elif label == '|':
                return max(self.Vio_Worst_Time(formula.left), self.Vio_Worst_Time(formula.right))
            elif label == '!':
                return self.Sat_Worst_Time(formula.left)

            elif re.search(r'F\[\d,\d\]',label):
                  pattern = r'\d+'
                  result = [int(s) for s in re.findall(pattern, label)]
                  
                  return self.Vio_Worst_Time(formula.left)+result[1]
