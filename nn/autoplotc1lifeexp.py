...
df_avg = df.groupby(['year', 'gnd'])[['hh_inc', 'le_raceadj']].mean().reset_index()
ts_s1=df_avg[df_avg['gnd'] == 'F']
df_avg2 = df.groupby(['year'])[['hh_inc', 'le_raceadj']].mean().reset_index()

fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(9, 4), sharey=True)
pd.plotting.autocorrelation_plot(ts_s1['le_raceadj'], ax=axes[0])
pd.plotting.autocorrelation_plot(df_avg2['le_raceadj'], ax=axes[1])
