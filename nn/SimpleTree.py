...
class Formula(SimpleTree):
    ...
    def normalize(cls, f):
        ...
        if fLeft.label == "true":
            if f.label in ['|', r'F\[\d+,\d+\]', r'G\[\d+,\d+\]', 'X']:
                return Formula("true")
            if f.label in ["&", "->"]:
            ...
            if f.label == r'U\[\d+,\d+\]':
                return Formula.normalize(Formula([r"F\[\d+,\d+\]", fRight, None]))
        ...
        if f.label ==  r'U\[\d+,\d+\]':
            if fLeft.label == r'F\[\d+,\d+\]' or fLeft.label == '!':
                fLeftLeft = Formula.normalize(fLeft.left)
                if fLeftLeft == fRight:
                    return Formula.normalize(Formula([r'F\[\d+,\d+\]', fLeftLeft]))
            if fRight.label == r'F\[\d+,\d+\]':
                fRightLeft = Formula.normalize(fRight.left)
                if fRightLeft == fLeft:
                    return fRight

        if f.label == r'F\[\d+,\d+\]' and fLeft.label == r'F\[\d+,\d+\]':
            return fLeft
    ...
    def convertTextToFormula(cls, formulaText):
        ...
        try:
            formula_parser = Lark(r"""
                ...
                variable: /x[0-9]*>(\d*\.)?\d+/ |  /x[0-9]*<(\d*\.)?\d+/
                !binary_operator: "&" | "|" | "->" | /U\[\d,\d\]/ 
                !unary_operator: /F\[\d+,\d+\]/ | /G\[\d+,\d+\]/ | "!" | "X"
